#!/usr/bin/env node
/**
 * Copy the canonical repo-root `ayepi.md` into this package so it ships with `@ayepi/docs`.
 *
...
const src = join(here, '..', '..', '..', 'ayepi.md'); // packages/docs/scripts -> repo root
...
console.log(`bundle-index: copied ayepi.md into @ayepi/docs`);
